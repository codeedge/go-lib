...
// queueName: 队列名称
// data: JSON可序列化的数据
func (r *Rabbit) PublishToQueue(ctx context.Context, queueName string, data any) error {
	body, err := json.Marshal(data)
	if err != nil {
		return fmt.Errorf("json marshal failed: %w", err)
	}

	return r.Publish(ctx, &PublishOption{
		Exchange:    "",
...
// exchangeName: 交换机名称
// data: JSON可序列化的数据
func (r *Rabbit) PublishToFanout(ctx context.Context, exchangeName string, data any) error {
	// 显式声明交换机，确保不会因为交换机不存在而丢消息
	if err := r.DeclareExchange(ExchangeOption{
		Name:    exchangeName,
		Kind:    "fanout",
		Durable: true,
	}); err != nil {
...
// data: JSON可序列化的数据
func (r *Rabbit) PublishToDirect(ctx context.Context, exchangeName, routingKey string, data any) error {
...
// routingKey: 路由键（Direct交换机精确匹配/Direct交换机支持通配符）
// data: JSON可序列化的数据
func (r *Rabbit) PublishToTopic(ctx context.Context, exchangeName, routingKey string, data any) error {
	// 显式指定为 topic 类型
	if err := r.DeclareExchange(ExchangeOption{
		Name:    exchangeName,
		Kind:    "topic",
...
func unifiedMessageHandler(queueName string, handler func(data []byte) error) func(d amqp.Delivery) {
	return func(d amqp.Delivery) {
		...
		if err := handler(body); err != nil {
			log.Printf("rabbitmq-log:Message handler failed for queueName:%s Exchange:%s RoutingKey:%s body:%s err:%v\n", queueName, d.Exchange, d.RoutingKey, string(body), err)
			...
			if errors.As(err, &permErr) && permErr.Permanent() {
				...
				log.Printf("rabbitmq-log:Message treated as permanent failure, sending to DLX.queueName:%s Exchange:%s RoutingKey:%s body:%s\n", queueName, d.Exchange, d.RoutingKey, string(body))
			}
			...
			// 业务处理成功，Ack
			d.Ack(false)
			log.Printf("rabbitmq-log:Message treated as successful.queueName:%s Exchange:%s RoutingKey:%s body:%s\n", queueName, d.Exchange, d.RoutingKey, string(body))
		}
	}
}
...
func (r *Rabbit) ConsumeFromQueue(ctx context.Context, queueName string, autoAck bool, handler func(data []byte) error) error {
	// 检查队列名称是否为空
	if queueName == "" {
	...
	return r.Consume(ctx, &ConsumeOption{
		Queue:   queueName,
		AutoAck: autoAck,
...
func (r *Rabbit) ConsumeFromFanout(ctx context.Context, exchangeName, queueName string, autoAck bool, handler func(data []byte) error) error {
	...
	// 3. 声明队列
	if err := r.DeclareQueue(opt); err != nil {
		return fmt.Errorf("declare queue failed: %w", err)
	}
	...
	// 绑定队列到Fanout交换机
	if err := r.BindQueue(queueName, "", exchangeName); err != nil {
		return fmt.Errorf("bind queue failed: %w", err)
	}

	// 开始消费
	return r.ConsumeFromQueue(ctx, queueName, autoAck, handler)
}
...
// autoAck: 是否自动确认
// handler: 消息处理函数
func (r *Rabbit) ConsumeFromDirect(ctx context.Context, exchangeName, queueName, routingKey string, autoAck bool, handler func(data []byte) error) error {
	...
	// 绑定队列到Direct交换机
	if err := r.BindQueue(queueName, routingKey, exchangeName); err != nil {
		return fmt.Errorf("bind queue failed: %w", err)
	}
...
// autoAck: 是否自动确认
// handler: 消息处理函数
func (r *Rabbit) ConsumeFromTopic(ctx context.Context, exchangeName, queueName, routingKey string, autoAck bool, handler func(data []byte) error) error {
	if queueName == "" || routingKey == "" {
		return errors.New("queueName and routingKey cannot be empty")
	}
	// 声明Topic交换机
...
func (r *Rabbit) PublishDelay(ctx context.Context, targetQueue string, data any, delay time.Duration) (err error) {
	// 1. 定义死信交换机和死信路由键 (这些是消息过期后流向的地方)
	// 约定死信交换机名（基于目标队列名拼接，保证通用性）
	dlxExchange := "dlx.ex." + targetQueue

	// 在这里显式声明一下死信交换机
	// 虽然消费者会声明，但如果生产者先启动，消息过期后若交换机不存在，消息会丢失。
	err = r.DeclareExchange(ExchangeOption{Name: dlxExchange, Kind: "direct", Durable: true})
	if err != nil {
		return err
	}
	...
	err = r.DeclareQueue(&QueueOption{
		Name:    delayQueueName,
		Durable: true,
		Args: amqp.Table{
...
func (r *Rabbit) ConsumeDelay(ctx context.Context, targetQueue string, handler func(data []byte) error) error {
	dlxExchange := "dlx.ex." + targetQueue
	...
	// 2. 声明最终处理队列并绑定到死信交换机
	if err := r.DeclareQueue(&QueueOption{Name: targetQueue, Durable: true}); err != nil {
		return err
	}

	// 3. 绑定死信交换机和处理队列
	if err := r.BindQueue(targetQueue, targetQueue, dlxExchange); err != nil {
		return err
	}

	// 4. 开始消费处理队列
	return r.ConsumeWorkQueue(ctx, targetQueue, "delay-worker", PrefetchCount, handler)
}
...
func (r *Rabbit) PublishDelayPlugin(ctx context.Context, exchangeName, routingKey string, data any, delay time.Duration) error {
	body, err := json.Marshal(data)
	if err != nil {
		return err
	}
	...
	pub := amqp.Publishing{
		...
		ContentType:  Json,
		Body:         body,
		DeliveryMode: amqp.Persistent, // 延迟消息必须持久化，否则插件存内存，重启就没了
	}

	// 2. 如果全局配置没开启 Confirm，直接发送返回
	if !r.config.EnablePublisherConfirm {
		return ch.PublishWithContext(ctx, exchangeName, routingKey, false, false, pub)
	}

	// 3. 如果开启了 Confirm，走确认逻辑（参考你 doPublish 的写法）
	...
	case <-time.After(5 * time.Second): // 5秒超时逃生门
		channelClosed = true
...
func (r *Rabbit) ConsumeDelayPlugin(ctx context.Context, exchangeName, queueName, routingKey string, handler func(data []byte) error) error {
	...
	// 2. 正常绑定和消费
	if err := r.DeclareQueue(&QueueOption{Name: queueName, Durable: true}); err != nil {
		return err
	}
	r.BindQueue(queueName, routingKey, exchangeName)

	return r.ConsumeWorkQueue(ctx, queueName, "plugin-delay-worker", PrefetchCount, handler)
}
...
它会先向机器 A 发送一个 Basir.Cancel（取消通知），停止给 A 发送队列 1-10 的新消息。
...
func (r *Rabbit) PublishToHashExchange(ctx context.Context, exchangeName, hashKey string, data any) error {
	// 1. 声明哈希交换机
	err := r.DeclareExchange(ExchangeOption{
		Name:    exchangeName,
...
// handler: 业务处理函数
func (r *Rabbit) ConsumeFromHashExchange(ctx context.Context, exchangeName, queueName, weight string, handler func(data []byte) error) error {
	// 1. 声明哈希交换机
	if err := r.DeclareExchange(ExchangeOption{
		Name:    exchangeName,
	...
	if err := r.BindQueue(queueName, weight, exchangeName); err != nil {
		return err
	}

	// 4. 调用你封装好的工作队列模式开始消费
	return r.ConsumeWorkQueue(ctx, queueName, "hash-worker", PrefetchCount, handler)
}
...
// data: 业务数据
func (r *Rabbit) PublishToLogicHash(ctx context.Context, exchangeName, routingKeyPrefix string, id int64, queueCount int, data any) error {
	...
	return r.PublishToDirect(ctx, exchangeName, routingKey, data)
}
...
// handler: 业务处理函数
func (r *Rabbit) ConsumeFromLogicHash(ctx context.Context, routingKeyPrefix string, queueIdx int, prefetchCount int, handler func(data []byte) error) error {
	...
	return r.ConsumeWorkQueue(ctx, queueName, consumerTag, prefetchCount, handler)
}

// SetupLogicHashQueues 辅助方法：初始化所有 Hash 物理队列并绑定到交换机
// 在消费者程序启动时调用一次
func (r *Rabbit) SetupLogicHashQueues(exchangeName, routingKeyPrefix string, queueCount int) error {
	for i := 0; i < queueCount; i++ {
		...
		// 2. 绑定路由键 (路由键名 = 队列名)
		if err := r.BindQueue(queueName, queueName, exchangeName); err != nil {
			return err
		}
	}
	return nil
